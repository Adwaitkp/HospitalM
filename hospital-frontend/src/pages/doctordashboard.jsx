...
const DoctorDashboard = () => {
  ...
  const fetchAssignedAppointments = async () => {
    try {
      const token = localStorage.getItem("doctorToken");
      const res = await axios.get("http://localhost:5000/api/doctor/appointments/assigned", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchTodayAppointments = async () => {
    try {
      const token = localStorage.getItem("doctorToken");
      const res = await axios.get("http://localhost:5000/api/doctor/appointments/assigned/today", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchPrescriptionData = async (appointmentId) => {
    try {
      const token = localStorage.getItem("doctorToken");
      const res = await axios.get(`http://localhost:5000/api/doctor/appointments/${appointmentId}/prescription`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      return res.data;
    } catch (err) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const token = localStorage.getItem("doctorToken");
      const res = await axios.get(`http://localhost:5000/api/doctor/appointments/search?name=${searchQuery}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handlePrescribe = async (prescription) => {
    ...
    try {
      const token = localStorage.getItem("doctorToken");
      await axios.post(`http://localhost:5000/api/doctor/appointments/prescribe/${selectedAppointmentId}`, {
        prescription
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
